...
class GlowOverlay(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Create multiple gradient layers for a more realistic glow effect
        rect = self.rect()
        center_x = rect.width() // 2
        
        # Outer glow (most diffuse)
        outer_glow = QtGui.QRadialGradient(center_x, rect.height(), rect.width() * 0.8)
        outer_glow.setColorAt(0, QtGui.QColor(138, 43, 226, 30))  # Purple with low opacity
        outer_glow.setColorAt(0.3, QtGui.QColor(138, 43, 226, 15))
        outer_glow.setColorAt(1, QtCore.Qt.transparent)
        painter.setBrush(outer_glow)
        painter.setPen(QtCore.Qt.NoPen)
        painter.drawRect(rect)
        
        # Middle glow (medium intensity)
        middle_glow = QtGui.QRadialGradient(center_x, rect.height(), rect.width() * 0.5)
        middle_glow.setColorAt(0, QtGui.QColor(138, 43, 226, 80))  # Brighter purple
        middle_glow.setColorAt(0.4, QtGui.QColor(138, 43, 226, 40))
        middle_glow.setColorAt(1, QtCore.Qt.transparent)
        painter.setBrush(middle_glow)
        painter.drawRect(rect)
        
        # Inner glow (brightest core)
        inner_glow = QtGui.QRadialGradient(center_x, rect.height(), rect.width() * 0.25)
        inner_glow.setColorAt(0, QtGui.QColor(186, 85, 211, 120))  # Bright purple core
        inner_glow.setColorAt(0.2, QtGui.QColor(138, 43, 226, 90))
        inner_glow.setColorAt(0.6, QtGui.QColor(138, 43, 226, 30))
        inner_glow.setColorAt(1, QtCore.Qt.transparent)
        painter.setBrush(inner_glow)
        painter.drawRect(rect)
        
        # Add a subtle edge highlight
        edge_highlight = QtGui.QLinearGradient(0, rect.height() - 2, 0, rect.height())
        edge_highlight.setColorAt(0, QtGui.QColor(186, 85, 211, 150))
        edge_highlight.setColorAt(1, QtGui.QColor(138, 43, 226, 200))
        painter.setBrush(edge_highlight)
        painter.drawRect(0, rect.height() - 3, rect.width(), 3)
